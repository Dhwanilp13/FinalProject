...
try:
    # Connect to database using local settings.ini (not committed)
        ...
        for (id, name, gene_class, mechanism, source, sequence) in cursor:
            try:
                ...
                if len(aligned_seqs) >= 3:
                    ...
                    # Count matches and alignment length (excluding gaps)
                    matches = 0
                    alignment_length = 0
                    for q, t in zip(aligned_query, aligned_target):
                        if q != '-' and t != '-':  # Only count positions where neither sequence has a gap
                            alignment_length += 1
                            if q == t:  # Count matches
                                matches += 1
                    
                    # Calculate identity percentage using alignment length without gaps
                    identity = (matches / alignment_length) * 100 if alignment_length > 0 else 0
                    
                    # Only process matches with >50% identity
                    if identity > 50:
                        # Find all fragments where query aligns to gene
                        fragments = []
                        current_start = None
                        gene_pos = 0  # Position in the gene sequence
                        
                        for q, t in zip(aligned_query, aligned_target):
                            if t != '-':  # Only increment gene position when not a gap in gene
                                if q != '-':  # Query has a base here
                                    if current_start is None:
                                        current_start = gene_pos
                                if current_start is not None and (q == '-' or t == '-'):
                                    # End of a fragment
                                    fragments.append(f"{current_start + 1}-{gene_pos}")
                                    current_start = None
                                gene_pos += 1
                        ...
                        # If no fragments were found (perfect continuous match), use whole aligned region
                        if not fragments:
                            fragments.append(f"1-{gene_pos}")
                        
                        # Join all fragments with commas
                        position_str = ", ".join(fragments)
